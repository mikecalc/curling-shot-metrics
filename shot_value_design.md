...
**How this document is organised.** Part I is the corpus: where it comes from, how it is extracted and validated, and how a position is described, from the raw stones to the configurations a skip reads (a split house, the steal is on, a runback on the shot rock). Part II is the studies the corpus has made possible so far and the ones it points to next. Part III is one application built on top: an expectation model for curling positions, f and g, which says what a position and a call have been worth under the field's play, and **Points Gained**, which values every stone as the change it made to that expectation. Points Gained is a useful way to think about positions and a way to rate execution; it is one lens on the corpus, not the reason for it. Part IV records open questions and status.
