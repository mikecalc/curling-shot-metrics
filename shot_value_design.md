...
- **Game report** (`pointsgained game`): one game from the per-shot table. An end table with the hammer team's expectation at the start, the result in the same currency, each team's summed call and throw values and the win probability before and after the end; a player table with sums over the game and each player's best and worst stone; the ten largest win-probability swings; and every stone with V before, V after the call and V after, in both currencies. It is the report that lets a leaderboard number be traced to the stones that produced it.

Every per-shot value is a difference of model outputs, so these tables are exactly reproducible from the Parquet tables and the fitted models. `reports/samples/` keeps finished copies in the repository (the model report, the test set, the 2026 Olympic and World Championship leaderboards, Beijing 2022, and the two 2026 Olympic finals shot by shot) with a README on how each is built and what it says. One pattern from those samples is recorded here because it bears on Section 6: in every event sampled, the teams with the best execution have negative call values and the teams with the worst have positive ones. The likely mechanism is game state, not judgement. A team in control calls the quiet shot, whose usual result is below the value of the position it was called from; a team that is chasing calls the hit, whose usual result is above it. The call component therefore carries situation as well as selection, one more reason it is reported second.
