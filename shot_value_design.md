...
Seven problems that the work so far runs into directly, stated for contributors. `open_problems.md` states these and three more (comparing fields, credit beyond the thrower, uncertainty) for coaches and players, in curling terms, with a question for coaches on each. Section 17 is the finer-grained list of known issues.
...
3. **The best call, and the skip against it.** PG: Call is measured against the field's usual call, not the best available one (Section 6.3); Jacobs' ninth-end runback in the 2026 final is the test case. A best call depends on geometry, the thrower's make rate and the value of each outcome in the game, and skips manage risk across several stones. Start: the empirical menu (calls the field made from similar positions, valued by g), then the transition model of Section 11.
