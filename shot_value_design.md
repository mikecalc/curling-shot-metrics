...
A position is a set of up to 16 stones, each (x, y, owner) in inches, plus rocks remaining and rule era. Two encodings will be tried: a **raster** (the play area as a two-channel image at about one inch per pixel, roughly 150 by 300, fed to a small convolutional network) and a **set** encoding (a permutation-invariant network over stones). Raster first: it is simpler to debug and the subtlety probe of Section 7.5 is easy to run on it. The archive's 609,014 shots, 1.2 million rows with mirroring, are enough to train it.
...
The delivered-stone marker (Section 3.3) is the way forward. On shots graded 100% the delivered stone's final position is the target, and which stones moved (the prior-position rings) is the intent. A target distribution per (type, position) learned from those shots, with lower-graded shots as noisy realisations of it, gives g a call encoding that includes the target, and gives Phase 2 its execution error model. The Through / Draw distinction on a last rock with an empty house remains the blank-versus-score decision and is handled by the type alone.
...
|---|---|---|---|---|---|---|---|
| Edin | 213 | 0.048 | 0.030 | −0.362 | 58.7% | 16 | 84.1 |
| Mouat | 206 | 0.046 | 0.056 | −0.341 | 57.8% | 10 | 89.0 |

Edin edges the median and reliability, Mouat the mean, the floor and the big-miss count; both separate from the other fourths on every column and from each other on none by much. Sweden won the final 5–4 in an extra end. The model's read of the tournament is the field's two most reliable fourths playing to a draw.
...
|---|---|---|---|---|---|---|---|
| Worlds 2024 | 0.110 | 0.132 | −0.256 | 66.0% | 9 | 33 | 88.5 |
| Worlds 2025 | 0.076 | 0.101 | −0.321 | 61.9% | 15 | 26 | 87.6 |
| Olympics 2026 | 0.055 | 0.061 | −0.464 | 58.7% | 19 | 23 | 77.7 |
...
- **Execution error model.** The only stochastic component. Learned from the delivered-stone marker: for each (type, inferred target, skill scalar, event effect), the distribution of where the thrown stone arrived relative to the target.
