...
The setup-battle column is the raw correlation, which is mostly the score (Section 13.3); the gates now also report it within the same score and ends left, where every model sits between 0.0 and 0.06.

**A split model, tried and not adopted (September 2026).** `model/split.py` values the free guard zone with its own models, on a lean description built around rock potential (Section 4.4), taught by the endgame model's value of the position at the end of the free guard zone rather than by the end's result, with a hard or a blended handover (`experiment --split-model fgz [--blend a,b]`). The lean setup matched the full feature set, so potential can stand in for the 26 hand-built features early in the end, and every variant valued early positions slightly better (with 12 or more rocks left, 1.6048 against 1.6068 on the time split). But every variant made credit worse: the see-saw returned (about −0.04) and seconds' agreement with their grades fell. Training every setup position on one distant point flattens the differences between consecutive setup positions, as the phase target did. The single model with the two-stones-ahead target stays; rock potential enters it as the feature set `potential_cb` (a small gain on both splits: f 1.4521 and 1.4571 against 1.4531 and 1.4580), adopted in the refit of September 2026 (cross-validated f 1.4665, g 1.4450, from 1.4677 and 1.4457; one end's conservation residual of 0.0056 comes from a rebuilt post position whose potential columns were copied from the pre-shot row, fixed for the next refit) and, mainly, as a reading of positions (the potential ledger, Section 13.4).
...
| The early end | What do lead and second values measure, and who wins the battle over the setup? | The first model see-sawed between consecutive stones; a nearer training target removed it. A team's setup result repeats across an event but, within the same score and ends left, barely relates to winning (0.01-0.06 on held-out events). | `frontend`, `experiment` | 13 |
...
The **setup battle** is the change in value from the empty sheet to the position after the free guard zone, from each team's view. It is small per end (a standard deviation of 2 percentage points of win probability, against 15 for the rest of the end) but persistent: a team's mean over an event repeats between halves of the event. Its raw correlation with the team's win rate is high (0.74 on the held-out 2025–26 events), but that is mostly the score: good teams lead more often, and the setup swing correlates 0.29 with the score difference. Within the same score and ends left, the correlation with win rate is 0.006 on the time split and 0.03 by book for the adopted model, and 0.05-0.06 for the split models of Section 7.6: the setup is a repeatable team trait that, averaged over an event, predicts little of winning on its own. Classified by which outcome the setup made more likely, ends sort as the vocabulary says: setups that moved towards two or more for the hammer team produced two or more 39% of the time; setups that moved towards a steal produced a steal 27% of the time.
...
**The potential ledger.** Rock potential (Section 4.4) read stone by stone: after every stone, each team's potential (its stones' chance of counting or covering a counter when the end is over, from where stones like them sit at that stage of the end, with a stone in front of or just behind a counter credited to whichever team the counter belongs to), the thrower's **build** (potential added to its own team) and **address** (potential taken from the other team), and the end's **temperature** (both teams' potential together). The game report prints it beside the values, and under each end header each team's potential after the free guard zone, after stone 12 and before the last stone: the setup, the middle game and the conversion. The event reports add a build-or-address table per team, relative to the field at the same stage of the end. It is the first measured form of aggressive and conservative play: building while leaving the other team's potential standing, against taking it away. At Milano Cortina 2026 the two dominant men's teams, Switzerland and Canada, built least and addressed most. The stone tables support the asymmetry skips describe: hammer stones in the house end up counting or covering about as often on the wing as in the centre lane (11-16% with eight or more rocks left), while non-hammer stones on the wings do so about half as often as in the centre lane (3-4% against 6-7%), because the hammer team always has the draw to the centre to fall back on.
...
**Studies.** Every study in Part III reads the same tables: `points_gained.parquet` (one row per stone, with the three outcome distributions, both currencies, the call and execution split, the thrower and the situation), `configurations.parquet` (the labels and measures of every position), the per-book extraction tables, and the feature cache. `model/frontend.py` is a worked example of a study module. The strategic situations of Section 15 and the open problems of Section 16.1 are open; a study that matches on the score, ends remaining and position and compares the choices is the natural next contribution, and finished reports belong in `reports/samples/` with a line in its README.


### 16.1 Open problems

Seven problems that the work so far runs into directly, stated for contributors. `open_problems.md` states these and three more (comparing fields, credit beyond the thrower, uncertainty) in plain curling terms, for readers outside the project. Section 17 is the finer-grained list of known issues.

1. **The value of an early position.** With twelve or more rocks left f is within 0.02 log-loss of the trivial model, the raster network lost to the trees in every band (Section 7.5), and the local target of Section 7.6 fixed the attribution of early values without improving their accuracy. Whether the early end is intrinsically unpredictable or the representation is missing what a skip reads is open. Start: the log-loss by rocks remaining, `pointsgained raster`, the scenario probes.
2. **Learning configurations from data.** The labels of Section 4.4 are hand-written tests with hand-set thresholds, some corrected by the corpus (Section 12.1). Can positions be grouped by what they lead to and what they leave the opponent, so that the vocabulary is discovered rather than written, recovering `split_flat` and `deuce_loose` and finding configurations nobody has named? Start: `configurations.parquet`, the outcome distributions, the survival table of the front-end report.
...
4. **Strategy and execution together.** A call's value depends on who throws it. The tail hypothesis (Section 10.3) says the best teams accept calls with a fat bad tail because they do not produce it. Separating the right call for a team from the right call for the field without circularity is open. Start: the downside-by-tercile check of Section 10.3, D(S | C) by team.
...
6. **Difficulty without selection.** Make rates measure shots as chosen: the angled runback shows no penalty because it is called only when it is on (Section 12.2). The correction needs every position where a shot was available, called or not, and a definition of available. Start: the double and runback tables, the configuration table.
7. **Rare, decisive situations.** The trees cannot carve out small high-stakes states: the peel miss in a tied last end (276 ends; realised steal 30%, model 23%) and the tight guard on a lonely steal stone (Section 13.4). Candidates: targeted features, monotone constraints, a situation-weighted correction. Start: the scenario probes as tests.
...
| M12 Stone tracking, rock potential (colour-blind), the potential ledger; split model tried | Done; adopted `potential_cb` in f and g, CV f 1.4665 / g 1.4450; split model not adopted; ledger in the game and event reports |
| M13 Studies: geometry by level, high-value-shot leaderboards, build-or-address studies, strategic situations | Next |
| M14 Phase 2 | After M13 |
